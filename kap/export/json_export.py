...
def export_ranking(df: pd.DataFrame, mode: str, regime_state: str, universe_size: int) -> Path:
    ...
    for _, r in df.iterrows():
        rec = {
            ...
            "change_pct": float(r.get("change_pct", float("nan"))) if "change_pct" in df.columns else None,
            "change_20d_pct": float(r.get("change_20d_pct", float("nan"))) if "change_20d_pct" in df.columns else None,
            "close_history": list(r.get("close_history", [])) if "close_history" in df.columns else [],
